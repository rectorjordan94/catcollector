...
# we want to use the built-in form for custom view for signup
...
def add_photo(request, cat_id):
    ...
    if photo_file:
        ...
        # we're going to use try...except which is just like try...catch in js. Handles the situation if anything should go wrong
        try:
            # if success
            s3.upload_fileobj(photo_file, S3_BUCKET, key)
            # build the full url string to upload to s3
            url = f"{S3_BASE_URL}{S3_BUCKET}/{key}"
            # if our upload(that used boto3) was successful we want to use that photo location to create a Photo model
